...
void ParseArgs(int& argc, char* argv[], TStr& InFile, TStr& OutFile,
 int& Dimensions, int& MaxWalkLen, int& NumWalks, int& WinSize, int& Iter,
 ...
 bool& OutputWalks) {
  ...
  MaxWalkLen = Env.GetIfArgPrefixInt("-l:", 80,
...
void WriteEdgesToFile(FILE *F, const PNEANet & Graph, TStrV & IntAttrENames,
		 TStrV & FltAttrENames,
		 TStrV & StrAttrENames) {
  for (TNEANet::TEdgeI EI = Graph->BegEI(); EI < Graph->EndEI(); EI++) {
	...
	fprintf(F, "\t%s", TSnap::NULL_VAL.CStr());
	continue;
      }
...
int main(int argc, char* argv[]) {
  TStr InFile,OutFile;
  int Dimensions, MaxWalkLen, NumWalks, WinSize, Iter;
  double ParamP, ParamQ;
  bool Directed, Weighted, Verbose, OutputWalks;
  ParseArgs(argc, argv, InFile, OutFile, Dimensions, MaxWalkLen, NumWalks, WinSize,
   Iter, Verbose, ParamP, ParamQ, Directed, Weighted, OutputWalks);
  TIntFltVH EmbeddingsHV;
  TVVec <TInt, int64> WalksVV;
  PNEANet InNet = TSnap::LoadEdgeListNet(InFile, '\t');
  if (Verbose) {
    printf("%d nodes in graph\n", InNet->GetNodes());
    printf("%d edges in graph\n", InNet->GetEdges());
  }
  ctdne(InNet, ParamP, ParamQ, Dimensions, MaxWalkLen, NumWalks, WinSize, Iter,
